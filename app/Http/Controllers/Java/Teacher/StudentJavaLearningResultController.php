...
class StudentJavaLearningResultController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->addColumn('hasil', function ($row) use ($user) {
                $totalScore = 0;
                $pattern = '/Total Score: (\d+)/';

                $topics = JavaExerciseTopic::with('javaExercise')->where('java_exercise_id', $row->id)->get();
                ...
                return $totalScore == 0 ? $totalScore : $totalScore  / $row->topicWorkedOn();
            });
            $table->editColumn('actions', function ($row) use ($user) {
                $button = '<a class="btn btn-warning mr-1" href="' . route('teacher.java.learning-result.show', ["javaExercise" => $row->id, "id_student" => $user->id]) . '"><i class="fa fa-eye"></i> </a>';
            ...
            $table->addColumn('topic_passed', function ($row) use ($user) {
                $topics = JavaExerciseTopic::with('javaExercise')->where('java_exercise_id', $row->id)->get();
                $topicsPassed = 0;
        ...
        return view('teacher.java.learningResult.index', compact('username', 'user'));
    }
    ...
    public function show(JavaExercise $javaExercise, Request $request)
    {
        ...
        return view('teacher.java.learningResult.show', compact('javaExercise', 'feedback', 'user'));
    }
    ...
    public function topicAdapter(Request $request, JavaExercise $javaExercise)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('trying_times', function ($row) use ($user) {
                $total = JavaExerciseTopicUser::with('javaExerciseTopic')
                    ->where('java_exercise_topic_id', $row->id)
                    ->where('user_id', $user->id)->get()->count();
                return $total;
            });

            $table->addColumn('results', function ($row) use ($user) {
                $totalScore = 0;
                $pattern = '/Total Score: (\d+)/';
                $report = JavaExerciseTopicUser::with('user')->where([
                    'java_exercise_topic_id' => $row->id,
        ...
        return redirect()->route('teacher.java.learning-result.index');
    }

    public function feedback(Request $request, JavaExercise $javaExercise)
    {
        return view('teacher.java.learningResult.feedback', compact('javaExercise'));
    }
